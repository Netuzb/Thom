...
class Module:
    ...
    def __getattr__(self, name: str):
        if name in {"thom_commands", "commands"}:
            return get_commands(self)

        if name in {"thom_inline_handlers", "inline_handlers"}:
            return get_inline_handlers(self)

        if name in {"thom_callback_handlers", "callback_handlers"}:
            return get_callback_handlers(self)

        if name in {"thom_watchers", "watchers"}:
