...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _thom_client_id_logging_tag = copy.copy(
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?thom$", from_id=659800858)
...
class Modules:
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.thom_commands)
                inline_handlers.update(module.thom_inline_handlers)
                callback_handlers.update(module.thom_callback_handlers)
                watchers.extend(module.thom_watchers.values())
    ...
    async def _register_modules(self, modules: list, origin: str = "<core>"):
        with contextlib.suppress(AttributeError):
            _thom_client_id_logging_tag = copy.copy(self.client.tg_id)
    ...
    def register_commands(self, instance: Module):
        ...
        if getattr(instance, "__origin__", "") == "<core>":
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.thom_commands))
            )
        ...
        for _command, cmd in instance.thom_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.thom_commands:
                self.add_alias(alias, cmd)

        for name, func in instance.thom_inline_handlers.copy().items():
        ...
        for name, func in instance.thom_callback_handlers.copy().items():
    ...
    def register_watcher(self, instance: Module):
        ...
        for _watcher in instance.thom_watchers.values():
    ...
    async def _decline(
        ...
        self._db.set(
            "thom.main",
            "declined_joins",
            list(set(self._db.get("thom.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def _request_join(
        ...
        if channel.id in self._db.get("thom.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        _module.strings._base_strings["_thom_internal_request_join"] = (
        ...
        _module.strings_ru["_thom_internal_request_join"] = (
        ...
        await self.inline.bot.send_animation(
            ...
            caption=_module.strings("_thom_internal_request_join"),
        ...
        _module.thom_wait_channel_approve = (
        ...
        with contextlib.suppress(AttributeError):
            delattr(_module, "thom_wait_channel_approve")
    ...
    async def complete_registration(self, instance: Module):
        ...
        instance.get_prefix = partial(self._db.get, "thom.main", "command_prefix", ".")
    ...
    async def _mod_import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?thom_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?thom_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Thom version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"thom.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("thom.main", "stats", True)
            and url is not None
        ...
        lib_obj.get_prefix = partial(self._db.get, "thom.main", "command_prefix", ".")
