...
class Database(dict):
    ...
    def _postgre_save_sync(self):
        with self._postgre, self._postgre.cursor() as cur:
            cur.execute(
                "UPDATE thom SET data = %s WHERE id = %s;",
    ...
    async def postgre_init(self) -> bool:
        ...
        with conn, conn.cursor() as cur:
            cur.execute("CREATE TABLE IF NOT EXISTS thom (id bigint, data text);")

            with contextlib.suppress(Exception):
                cur.execute(
                    "SELECT EXISTS(SELECT 1 FROM thom WHERE id=%s);",
                    (self._client.tg_id,),
                )

                if not cur.fetchone()[0]:
                    cur.execute(
                        "INSERT INTO thom (id, data) VALUES (%s, %s);",
            ...
            with contextlib.suppress(Exception):
                cur.execute(
                    ...
                    "WHERE table_name = 'thom' AND column_name = 'id';"
                )
                ...
                if "integer" in cur.fetchone()[0].lower():
                    ...
                    cur.execute("ALTER TABLE thom ALTER COLUMN id TYPE bigint;")
    ...
    def read(self):
        ...
        if self._postgre:
            try:
                with self._postgre, self._postgre.cursor() as cur:
                    cur.execute(
                        "SELECT data FROM thom WHERE id=%s;",
                        (self._client.tg_id,),
                    )
                    self.update(
                        **json.loads(
