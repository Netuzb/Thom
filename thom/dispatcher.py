...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.thom_me.id
        self._cached_username = (
            self._client.thom_me.username.lower()
            if self._client.thom_me.username
            else str(self._client.thom_me.id)
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("thom-")
            and message.chat.title != "thom-logs"
        ):
            if not watcher:
    ...
    async def future_dispatcher(
        ...
        *args,
        ...
        _thom_client_id_logging_tag = copy.copy(self.client.tg_id)  # skipcq
        try:
            await func(message)
