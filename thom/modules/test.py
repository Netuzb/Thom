...
class TestMod(loader.Module):
    ...
    strings = {
        ...
        "logs_filename": "thom-logs.txt",
        "logs_caption": (
            "<emoji document_id=5188377234380954537>🌘</emoji> <b>Thom logs with"
            " verbosity </b><code>{}</code>\n\n<emoji"
            " document_id=5454390891466726015>👋</emoji> <b>Thom version:"
    ...
    strings_ru = {
        ...
        "logs_filename": "thom-logs.txt",
        "logs_caption": (
            "<emoji document_id=5188377234380954537>🌘</emoji> <b>Логи Thom уровня"
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                " logs from TestModule. If this option is enabled, Thom will send all"
    ...
    async def logs(
        ...
        if btoken := self._db.get("thom.inline", "bot_token", False):
            logs = logs.replace(
                btoken,
                f'{btoken.split(":")[0]}:***************************',
            )

        if thom_token := self._db.get("ThomDL", "token", False):
            logs = logs.replace(
                thom_token,
                f'{thom_token.split("_")[0]}_********************************',
            )

        if thom_token := self._db.get("Kirito", "token", False):
            logs = logs.replace(
                thom_token,
                f'{thom_token.split("_")[0]}_********************************',
            )
        ...
        if os.environ.get("thom_session"):
            logs = logs.replace(
                os.environ.get("thom_session"),
                "StringSession(**************************)",
            )
        ...
        other = (
            *main.__version__,
            " <i><a"
            f' href="https://github.com/Netuzb/Thom/commit/{ghash}">({ghash[:8]})</a></i>'
            if ghash
            else "",
