...
class LoaderMod(loader.Module):
    ...
    strings = {
        ...
        "inline_init_failed": (
            ...
            " Thom inline feature and initialization of InlineManager"
    ...
    strings_ru = {
        ...
        "inline_init_failed": (
            ...
            " ThomInline, а инициализация менеджера инлайна неудачна</b>\n<i>Попробуй"
    ...
    async def _send_stats(self, url: str, retry: bool = False):
        """Send anonymous stats to Thom"""
        try:
            if not self.get("token"):
                self.set(
                    "token",
                    (
                        await (await self._client.get_messages("@thom_ub", ids=[10]))[
                            0
                        ].click(0)
                    ).message,
                )
            ...
            res = await utils.run_sync(
                ...
                data={"url": url},
                headers={"X-Thom-Token": self.get("token")},
            )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?thom_min", doc):
            ver = re.search(r"# ?scope: ?thom_min ((\d+\.){2}\d+)", doc).group(1)
        ...
        module_name = f"thom.modules.{uid}"
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "thom_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.thom_wait_channel_approve
        ...
        try:
            if developer in self._client._thom_entity_cache and getattr(
