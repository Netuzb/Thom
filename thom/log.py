...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(self, lvl: int = 0, client_id: Optional[int] = None) -> list:
        ...
        return [
            ...
            and (not record.thom_caller or client_id == record.thom_caller)
        ]
    ...
    async def sender(self):
        ...
        self._exc_queue = {
            client_id: [
                ...
                for item in self.tg_buff
                if isinstance(item[0], ThomException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_thom_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_thom_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
