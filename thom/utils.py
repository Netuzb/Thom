...
async def answer(
    ...
    **kwargs,
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "thom_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "thom")
        except Exception:
            folder = None
...
def get_commit_url() -> str:
    """Get current Thom git commit url"""
    try:
        repo = git.Repo()
        hash_ = repo.heads[0].commit.hexsha
        return (
            f'<a href="https://github.com/Netuzb/Thom/commit/{hash_}">#{hash_[:7]}</a>'
        )
    except Exception:
        return "Unknown"
...
def get_git_info():
    ...
    return [
        ver,
        f"https://github.com/Netuzb/Thom/commit/{ver}" if ver else "",
    ]
