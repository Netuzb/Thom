...
class Thom:
        ...
        if "DYNO" not in os.environ:
            self.sessions += [
                ...
                for session in filter(
                    lambda f: f.startswith("thom-") and f.endswith(".session"),
                    os.listdir(self.arguments.data_root or BASE_DIR),
                )
            ]

        for key, value in os.environ.items():
            if key.startswith("thom_session"):
    ...
    async def save_client_session(
        ...
        if "DYNO" not in os.environ:
            ...
            key = (
                f"thom_session_{utils.rand(6)}"
                if "thom_session" in heroku_config
                else "thom_session"
            )
        ...
        client.thom_db = database.Database(client)
        await client.thom_db.init()
    ...
    async def _badge(self, client):
        ...
        try:
            ...
            logo1 = f"""
                     ...
                     • Bu yagona O`zbekistondagi TOP 1 userbot 
                     • Build: {build[:7]}
            ...
            if not self.omit_log:
                ...
                logging.info(
                    f"🌘 Thom {'.'.join(list(map(str, list(__version__))))} started\n"
