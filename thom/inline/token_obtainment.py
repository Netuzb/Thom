...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("thom.inline", "custom_bot", False):
                username = self._db.get("thom.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@thom_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("thom.inline", "bot_token", token)
                    self._token = token
