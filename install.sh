...
runin() {
	...
	{ "$@" 2>>../thom-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../thom-install.log
	done
}
...
touch thom-install.log
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" thom-install.log
fi
...
if [ -f ".setup_complete" ] || [ -d "thom" -a ! x"" = x"$DYNO" ]; then
	# If thom is already installed by this script, or its in Heroku and installed
	PYVER=""
...
echo "Installing..." >thom-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/hikariatama/Thom/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Thom
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/hikariatama/Thom/ || {
